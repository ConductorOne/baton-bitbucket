...
func (r *repositoryResourceType) Grants(ctx context.Context, resource *v2.Resource, token *pagination.Token) ([]*v2.Grant, string, annotations.Annotations, error) {
	...
	workspaceId, _, repoId, err := splitRepositoryResourceID(resource.Id)
	if err != nil {
		return nil, "", nil, err
	}
...
func (r *repositoryResourceType) Revoke(ctx context.Context, grant *v2.Grant) (annotations.Annotations, error) {
	...
	// parse the grant ID to get workspace and repository id
	parts := strings.Split(entitlement.Resource.Id.Resource, ":")
	if len(parts) != 2 {
		return nil, fmt.Errorf("bitbucket-connector: invalid grant resource id: %s", entitlement.Resource.Id.Resource)
	}
